...
class ProcessCMD(Cmd):
    ...
    cmd_start_flask =  general_path_project+'/src/'+'platform_status_mon.py'
    ...
    cmd_start_aggr_naiv = 'python '+general_path_project+'/src/rta_wrapper.py -sh "'+general_path_project+'" -sn "har_predict_aggr_ensemble.py naivebayes"'+' 1>../log/'+aggr_log+' 2>&1'
    ...
    def do_start(self,processor):
        ...
        if processor == 'spark':
            if running_dict:
                ...
                print('You don\'t have permmission to run command from this host.You may use "help start" see more help')
                return
            ...
            if running_dict.has_key('flask') and running_dict['flask'] != 'Running':
                try:
                    cmd_line = 'python '+self.cmd_start_flask
                    print('staring flask webserver------->')
                    print cmd_line
                    cmd = subprocess.Popen([cmd_line],shell=True,stderr=subprocess.PIPE)
                    (output,err) = cmd.communicate()
                    ...
                    # app_logger.info('*********output logging **************')
                    # print(output)
                    print('flask webserver has been started!')
                except Exception as ex:
                    print("    Failed to run processor with ERROR(%s).", str(ex))
                    sys.exit(1)
            else:
                if not running_dict.has_key('flask'):
                    print('You don\'t have permmission to run command from this host.')
        ...
        if processor == 'spark':
            if running_dict.has_key('spark<spark_master>') and running_dict.has_key('spark<spark_master>') and (running_dict['spark<spark_master>']=='Running' or running_dict['spark<spark_worker>']=='Running'):
                ...
                if not (running_dict.has_key('spark<spark_master>') or running_dict.has_key('spark<spark_master>')):
                    print('You don\'t have permmission to run command from this host.You may use "help stop" see more help')
                    return
            ...
            if running_dict.has_key('kafka') and running_dict['kafka'] == 'Running':
                ...
                if not running_dict.has_key('kafka'):
                    print('You don\'t have permmission to run command from this host.please check with your administrator')
                    return
                else:
            ...
            if running_dict.has_key('flask') and running_dict['flask'] == 'Running':
                try:
                    ...
                    if not pid:
                        print ("No flask processor need to stop")
                        # sys.exit(1)
                ...
                if not running_dict.has_key('flask'):
                    ...
                    print('flask is already stopped! No action need to do!')
                    return
